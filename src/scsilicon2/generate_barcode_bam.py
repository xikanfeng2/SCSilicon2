...
def parse_args():
    ...
    for inp in inputs:
        if not (os.path.exists(inp) and not os.path.isdir(inp)):
            raise ValueError(print("This input file does not exist: {}".format(inp)))
    
    if not os.path.isdir(args.rundir):
        raise ValueError(print("Running directory does not exists: {}".format(args.rundir)))
    if args.reference is not None:
        if not os.path.isfile(args.reference):
            raise ValueError(print("Reference genome file does not exist: {}".format(args.reference)))
        refidx = ['{}.{}'.format(args.reference, ix) for ix in ['amb', 'ann', 'bwt', 'pac', 'sa']]
        if not all(os.path.isfile(f) for f in refidx):
            raise ValueError(print("Some of the BWA index files are missing, please make sure these are available and generated through the command ``bwa index FASTA-REFERENCE''. Expected files are: {}".format('\n'.join(refidx))))

    if not args.jobs:
        args.jobs = mp.cpu_count()
    if args.jobs < 1:
        raise ValueError(print("The number of jobs must be positive!"))

    bcftools = args.bcftools
    if not bcftools:
        bcftools = "bcftools"
    if shutil.which(bcftools) is None and not args.barcodeonly:
        raise ValueError(print("bcftools has not been found or is not executable!\n\nIf you are within a CHISEL conda environment ${ENV} you can install it with:\n\tconda install -c bioconda -n ${ENV} bcftools\n\nOtherwise, please provide with the flag `--bcftools` the full path to the directory containing bcftools exacutable."))

    samtools = args.samtools
    if not samtools:
        samtools = "samtools"
    if shutil.which(samtools) is None and not args.barcodeonly:
        raise ValueError(print("samtools has not been found or is not executable!\n\nIf you are within a CHISEL conda environment ${ENV} you can install it with:\n\tconda install -c bioconda -n ${ENV} samtools\n\nOtherwise, please provide with the flag `--samtools` the full path to the directory containing samtools exacutable."))

    bwa = args.bwa
    if not bwa:
        bwa = "bwa"
    if shutil.which(bwa) is None and not args.barcodeonly:
        raise ValueError(print("bwa has not been found or is not executable!\n\nIf you are within a CHISEL conda environment ${ENV} you can install it with:\n\tconda install -c bioconda -n ${ENV} bwa\n\nOtherwise, please provide with the flag `--bwa` the full path to the directory containing bwa exacutable."))
...
def main():
    ...
    if os.path.exists(tmpdir):
        raise ValueError(print("Temporary directory {} already exists, please move or rename it!".format(tmpdir)))
    os.mkdir(tmpdir)
    errdir = os.path.join(args['rundir'], '_ERR_CHISEL_PREP')
    if os.path.exists(errdir):
        raise ValueError(print("Temporary error directory {} already exists, please move or rename it!".format(errdir)))
    os.mkdir(errdir)
    ...
    if len(args['inputs']) == 1 and args['inputs'][0][-4:] != '.bam'\
        ...
        print('Reading provided table of inputs', )
    ...
    if all(f[-6:] == '.fastq' or f[-9:] == '.fastq.gz' for f in args['inputs']):
        if args['reference'] is None and not args['barcodeonly']:
            ...
            raise ValueError(print('Reference genome is required when running in FASTQ mode!'))
        if info is None:
            files, fastqinfo, ispaired = match_fastq(args['inputs'], args)
        else:
            files, fastqinfo, ispaired = make_fastqinfo(args['inputs'], info, args)
        if ispaired:
            print('Running in paired-end FASTQ mode', )
        else:
            print('Running in single-end FASTQ mode', )
        ...
        raise ValueError(print("Input files are of wrong format or mixed formats"))

    if not args['barcodeonly']:
        print('Indexing and finalizing final barcoded BAM file', )
        indexing(args['samtools'], args['jobs'], tmpdir, errdir, barcoded, args['output'])

        print('Retrieving stats about the resulting barcoded BAM file', )
        ...
        print('{}'.format(stdout), level='INFO')
    
    floginfo = os.path.join(args['rundir'], '{}.info.tsv'.format(args['output'][:-4]))
    print('Writing final summary of barcoded cells', )
    print('Number of barcoded cells: {}'.format(len(set(c[-1] for c in cells))), level='INFO')
    with open(floginfo, 'w') as o:
        o.write('{}\n'.format(header))
        for l in loginfo:
            o.write('{}\n'.format('\t'.join(map(str, l))))
    print('Final summary is written in {}'.format(floginfo), level='INFO')

    if not args['keeptmpdir']:
        print('Cleaning remaining temporary files', )
        shutil.rmtree(tmpdir)
        shutil.rmtree(errdir)
...
def read_table(file):
    ...
    if None in exts:
        raise ValueError(print('Unknown format, different than .bam, .fastq, or .fastq.gz has been provided!'))
    wrong = filter(lambda r : len(r) < 2, read)
    if len(wrong) > 0:
        raise ValueError(print('When a TSV is provided, every row must have at least two fields, specifying FILE and CELL-NAME, but an error was found, e.g.:\n\n{}'.format(wrong[0])))
    ...
    if {'b'} == exts:
        ...
        raise ValueError(print('The provided table contains files of mixed format (i.e. both BAM and FASTQ)!'))
...
def match_fastq(inputs, args):
    ...
    if all(None not in m for m in match):
        ...
        print('The filesnames of the provided FASTQ files do not match the format with given expressions and will be all considered as independent single-end FASTQs', level='WARN')
    ...
    if all(len(pairs[p]) == 1 for p in pairs):
        ...
        for p in pairs:
            if set(len(pairs[p])) < 2:
                raise ValueError(print('Found more files from the same cell with same filenames and identical reads!\n{}'.format(','.join(pairs[p]))))
            elif len(pairs[p]) > 2:
                raise ValueError(print('Found more than TWO files with the same cell and filenames, which cannot indicate paired-end reads!\n{}'.format(','.join(pairs[p]))))
            else:
                assert False
...
def run_q(args, tmpdir, errdir, files, fastqinfo, barcodeonly):
    ...
    if not barcodeonly:
        if args['noduplicates']:
            print('Alignment, barcoding and sorting is running for every cell', )
            bams = align(**par)
        else:
            print('Alignment, barcoding, sorting, and marking duplicates is running for every cell', )
            bams = align_marked(**par)

        print('Merging all cells', )
...
def run_b(args, tmpdir, errdir, binfo, barcodeonly):
    print('Running in multiple BAM files mode', )
    ...
    if not barcodeonly:
        if args['noduplicates']:
            print('Barcoding and sorting is running for every cell', )
            bams = barcode(**par)
        else:
            print('Barcoding, sorting, and marking duplicates is running for every cell', )
...
def run_B(args, tmpdir, errdir):
    print('Running in single BAM file mode', )
    
    print('Splitting reads in BAM file by RG tag', )
    ...
    if proc.returncode != 0:
        raise ValueError(print('Merging failed with messages:\n{}\n{}\n'.format(stdout, stderr)))
...
def mkbarcodes(files, length, info):
    ...
    if len(dup) > 0:
        raise ValueError(print('Two or more of these files have the same cell name but also the same rep number:\n{}'.format('\n'.join([f for f in reps[dup[0]]]))))
...
def indexing(samtools, jobs, tmpdir, errdir, barcoded, output):
    ...
    stdout, stderr = proc.communicate()
    if proc.returncode != 0:
        raise ValueError(print('Indexing failed with messages:\n{}\n{}\n'.format(stdout, stderr)))
    return


def error_code(e, c, errdir):
    raise ValueError(print('Commands failed for {} with error:\n{}\nCheck the errors in the corresponding log files in:\n{}'.format(e, c, errdir)))
